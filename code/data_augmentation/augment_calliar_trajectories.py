...
TRANSFORM_NAMES = ("scaling", "rotation", "shear", "noise", "time_shift", "elastic")
...
class AugmentConfig:
    ...
    elastic_alpha_min: float = 8.0   # displacement magnitude (canvas units)
    elastic_alpha_max: float = 24.0
    elastic_sigma: float = 6.0       # smoothness of the displacement field
    decimals: int = 3
    time_decimals: int = 3
...
def apply_elastic(arrays, _center, rng, config):
    """
    Elastic deformation via a smooth random displacement field.

    A low-frequency displacement field is generated by smoothing uniform noise
    with a Gaussian kernel (sigma controls spatial frequency). Each point is
    then displaced by bilinear interpolation into that field.

    This simulates the natural non-linear distortion of handwriting on paper —
    pen pressure variation, slight hand tremor, and substrate irregularities.
    Only x,y are affected; the t channel (if present) is left unchanged.
    """
    from scipy.ndimage import gaussian_filter

    alpha = float(rng.uniform(config.elastic_alpha_min, config.elastic_alpha_max))

    # Gather all points to compute a shared bounding box for the displacement grid
    all_pts = np.concatenate([a[:, :2] for a in arrays if len(a) > 0], axis=0)
    x_min, y_min = all_pts.min(axis=0) - 1.0
    x_max, y_max = all_pts.max(axis=0) + 1.0
    w = max(x_max - x_min, 1.0)
    h = max(y_max - y_min, 1.0)

    # Build displacement field on a coarse grid (32×32) then interpolate
    grid_size = 32
    raw_dx = rng.uniform(-1.0, 1.0, size=(grid_size, grid_size))
    raw_dy = rng.uniform(-1.0, 1.0, size=(grid_size, grid_size))
    smooth_dx = gaussian_filter(raw_dx, sigma=config.elastic_sigma) * alpha
    smooth_dy = gaussian_filter(raw_dy, sigma=config.elastic_sigma) * alpha

    # Bilinear sample the displacement field at each point's normalised position
    for array in arrays:
        nx = np.clip((array[:, 0] - x_min) / w * (grid_size - 1), 0, grid_size - 1)
        ny = np.clip((array[:, 1] - y_min) / h * (grid_size - 1), 0, grid_size - 1)
        ix, iy = nx.astype(int), ny.astype(int)
        fx, fy = nx - ix, ny - iy
        ix1 = np.clip(ix + 1, 0, grid_size - 1)
        iy1 = np.clip(iy + 1, 0, grid_size - 1)

        dx = (smooth_dx[iy, ix] * (1 - fx) * (1 - fy)
              + smooth_dx[iy, ix1] * fx * (1 - fy)
              + smooth_dx[iy1, ix] * (1 - fx) * fy
              + smooth_dx[iy1, ix1] * fx * fy)
        dy = (smooth_dy[iy, ix] * (1 - fx) * (1 - fy)
              + smooth_dy[iy, ix1] * fx * (1 - fy)
              + smooth_dy[iy1, ix] * (1 - fx) * fy
              + smooth_dy[iy1, ix1] * fx * fy)

        array[:, 0] += dx
        array[:, 1] += dy

    return {"type": "elastic", "alpha": alpha, "sigma": config.elastic_sigma}
